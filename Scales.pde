...
void octsqu(int x, int y){
  ...
  ellipse(x+(int)((7*(mouseX-x))/(Math.sqrt(Math.pow(mouseX-x,2)+Math.pow(mouseY-x,2))))%7,y+(int)(((7*(mouseY-y))/(Math.sqrt(Math.pow(mouseX-x,2)+Math.pow(mouseY-x,2)))))%7,15,15);
  fill(0,255,0);
